...
export function ParticipantApprovalPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (data.length > 0) setParticipant(data[0].user);
            } catch (error) {
    ...
    const handleMarkAttendance = async (regId, status) => {
        try {
            ...
            setRegistrations(registrations.map(r => r._id === regId ? { ...r, status } : r));
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            {registrations.map(reg => (
                                <div key={reg._id} className="sangamam-card p-6 border border-sangamam-border hover:border-sangamam-gold/50 transition-all group">
                                    <div className="flex justify-between items-start mb-6">
                                        <div>
                                            <h3 className="text-lg font-bold text-white group-hover:text-sangamam-gold transition-colors">{reg.event.name}</h3>
                                            <div className="flex items-center gap-4 mt-2 text-xs text-gray-500 font-medium">
                                                <span className="flex items-center gap-1"><Clock size={12} /> {reg.event.date}, {reg.event.time}</span>
                                                <span className="flex items-center gap-1"><MapPin size={12} /> {reg.event.venue}</span>
                                            </div>
                                        </div>
                                        <div className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border ${
                                            reg.status === 'participating' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' :
                                    ...
                                    <div className="flex gap-3">
                                        <button 
                                            onClick={() => handleMarkAttendance(reg._id, 'participating')}
                                        ...
                                        <button 
                                            onClick={() => handleMarkAttendance(reg._id, 'absent')}
                                            className={`flex-1 py-2.5 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all ${
                                                reg.status === 'absent' 
                        ...
                        <div className="sangamam-card p-8 border border-sangamam-border space-y-8 bg-gradient-to-br from-white/5 to-transparent">
                            <div className="flex items-center gap-6">
                                ...
                                <div>
                                    ...
                                    <p className="text-sangamam-gold text-xs font-bold uppercase tracking-[0.2em]">{participant.rollNo}</p>
                                </div>
                            </div>
